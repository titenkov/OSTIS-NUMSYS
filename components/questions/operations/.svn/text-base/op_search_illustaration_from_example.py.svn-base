...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/операция поиска иллюстрации от примера")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск иллюстрации объекта")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl15")

# класс операции поиска всех выходящих дуг
class Op_search_illustration_from_example(operation.Operation):
    
    # функция инициализации
    def __init__(self):
        print "init: Op_search_illustration_from_example"
        #nsm.initNSM(session)
    
    # функция проверки условия запуска
    @classmethod
    def checking(self, question):
        print "checking: Op_search_illustration_from_example"
    ...
    def running(self, question):
        print "runing: Op_search_illustration_from_example"
        res = []
