...
class GeometryEditor(BaseModeLogic):
    ...
    def createCircle(self, _point, _radius):
        """Create circle with center in specified point and known radius length
        @param _point: point object that specified circle center
        @type _point: GeometryPoint
        @param _radius: radius length
        @type _radius: float
        """
        circle_obj = gobjects.GeometryCircle()
        circle_obj.setCenterPoint(_point)
        circle_obj.setRadius(_radius)
        
        return circle_obj
    
    def createTriangle(self, _objects):
        """Create triangle object base on list of objects
        @param _object: list of objects
        @type _object: list 
        """
        triangle_obj = gobjects.GeometryTriangle(_objects)
