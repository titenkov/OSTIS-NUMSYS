...
class TranslatorSc2Text(Translator):
    ...
    def translate_impl(self, _input = None, _output = None):
        ...
        """
        errors = []        
        text = Translate(_input)
        sc_utils.setContentStr(session, segment, _output, str(text))
...
def Translate(sc_input):
    print "[sc2text] translate"
    answer = ""
    f = True
    rules = LoadRules()
    for rule in rules:
        ...
        out_pattern = GetOutputPattern(rule)
        if out_pattern is None: continue
        text = MakeAnswer(out_pattern, res_map)
        if f:
            answer = text
            f = False
        else:
...
def MakeAnswer(out_pattern, res_map):
    text = ""
    ff = True
    for map in res_map:
        t = out_pattern
        for key in map.keys():
            ...
            for el in map[key]:
                if f:
                    f = False
                    new = TranslateObject(el)
                else:
                    new = new + ", " + TranslateObject(el)
...
def GetOutputPattern(rule):
    sc_pattern = session.search5_f_a_a_a_f(rule, _aACP, sc.SC_N_CONST, _aACP, keys.patterns.attr_outputPattern)
    if sc_pattern is None:
        print "[sc2text] Error output pattern not found in %s" % session.get_idtf(rule)
        return None
    else:
        return session.get_content_str(sc_pattern[0][2])
