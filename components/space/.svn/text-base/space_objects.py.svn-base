...
class SpaceObject(objects.ObjectDepth):
    def __init__(self, sceneNode, parent = None, A = 0, B = 0, orbRelCenterPos = (0,0,0), year = 0, day = 0, title = None,
        ...
        self.day = float(day)
#        self.orbAnimState = None
#        self.spinAnimState = None
#        self.animation_orb = None
#        self.animation_spin = None
        self.__childObjects = []
...
#       вывод свойств объекта на консоль. для тестирования. можно удалить
#        print(title)
#        if properties:
#            for str in properties:
#                if str is not None:
#                    key, value, unit = str
...
#                        print(key +": ")
#                        for item in value:
#                            substance, amount = item
#                            print('\t'+substance +": "+ amount +" "+ unit)   
...
#        if self.spinAnimState:  sceneManager.destroyAnimationState(self.spinAnimState)
    ...
    def createAnimation(self):
        ...
        self.animation_orb = sceneManager.createAnimation(str(self)+"_orb", period)
        ...
        for i in range(pointNum):
            ...
            key.setTranslate(ogre.Vector3(self.orbA*math.cos(an), 0, self.orbB*math.sin(an)))
            an +=6.28/pointNum
        key = animationTrack.createNodeKeyFrame(period)
        key.setTranslate (ogre.Vector3(1,0,0)*self.orbA)
        
        self.orbAnimState = sceneManager.createAnimationState(str(self)+"_orb")
        ...
        self.animation_spin = sceneManager.createAnimation(str(self)+"_spin", period)
        animationTrack = self.animation_spin.createNodeTrack(1, node)
        for i in range(pointNum):
            key = animationTrack.createNodeKeyFrame(i*period/pointNum)        
            key.setRotation(ogre.Quaternion(ogre.Radian(i*6.28/pointNum), ogre.Vector3(0, 1, 0)))
        ...
        self.spinAnimState = sceneManager.createAnimationState(str(self)+"_spin")
        
            
    def animate(self):
        self.spinAnimState.setEnabled (True)
        render_engine.addAnimation(self.spinAnimState)
        self.orbAnimState.setEnabled (True)
        render_engine.addAnimation(self.orbAnimState)
        for obj in self.getChilds():
            obj.animate()
