...
#                    self.callBackRun(self)
    ...
    def _updateView(self):
        ...
        if self.needLocalizationUpdate:
            kenrle = core.Kernel.getSingleton()
            self.__caption = sc_utils.getIdentifier(kernel.session(), self._getScAddr(), kernel.getCurrentTranslation())
            if not self.__caption:
                self.__caption = "empty"
            self.__captionColored = self._color + self.__caption
            self.button.setCaption(self.__captionColored)
            self.calculateAutoSize()
            
            # reset localization update flag
            self.needLocalizationUpdate = False
            return # need to stop needViewUpdate flag goes to be false
    ...
    def show(self):
        ...
        if not self.__showed:
            ...
            if self._color:
                ...
                self.__captionColored = self.__caption
...
#            dx = 10
#            if not self.atom or self.question:
#                dx = 25
            self.setScale((tsize.width + 40, tsize.height + 10))
    ...
    def setCaption(self, caption):
        self.__caption = caption
        if self.button:
            self.button.setCaption(self.__caption)
...
class SCgMenuItem(Menu):
    ...
    def _parse(self):
        ...
        if decomp is not None:
            ...
            while not it.is_over():
                ...
                caption = sc_utils.getIdentifier(session, item_addr, current_translation)
                if not caption:
                    caption = "empty"
                                
                item = SCgMenuItem(caption, it.value(2), self)
    ...
    def show(self):
        ...
        if self.tooltip_value is not None:
            self.button.setNeedToolTip(True)
            
            self.button.subscribeEventToolTip(self, "_eventToolTip")
