...
class MouseMove(BaseCommand):
    ...
    def __init__(self, init_pos, target_pos):
        """Constructor
        
        @param init_pos:    initial mouse position
        @type init_pos:    (int, int)
        
        @param target_pos:    target mouse position
        @type target_pos:    (int, int)
        """
        ...
        self.dist = self.distance(init_pos, target_pos)
        self.speed = 200
    ...
    def _update(self, timeSinceLastFrame):
        ...
        rem_dist = self.distance(self.current_pos, self.target_pos)
        
        if dist >= rem_dist:    # end moving
            self.setMousePosition(self.target_pos)
            self.finish()
            return
