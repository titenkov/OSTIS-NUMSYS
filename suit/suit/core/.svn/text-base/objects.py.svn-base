...
class Object(ScObject):
    ...
    """
    # link semantics
    LS_IN, LS_OUT, LS_OTHER, LS_COUNT = range(4)
    ...
    def __init__(self):
        ...
        # linked objects
        self.linkedObjects = {self.LS_IN: [], self.LS_OUT: [], self.LS_OTHER: []}
        # flag to update
...
class ObjectOverlay(Object):
    ...
    def _updateView(self):
        ...
        if self.needTextUpdate:
            self._widget.setCaption(self.textValue)
            if self.autoSize:
                # calculate new size
...
#            self.sceneNode.resetOrientation()
    ...
    def _updateTextValue(self):
        ...
        if self.textValue and self.isSceneAttached:                
            ...
            self.text_obj.setText(self.textValue)
        else:
            self.text_obj = None
...
class ObjectLine(ObjectDepth):
    ...
    def _update(self, timeSinceLastFrame):
        ...
        if self.text_obj is not None: self.text_obj._update(timeSinceLastFrame)
...
class ObjectSheet(ObjectDepth, ois.KeyListener, ois.MouseListener):
    ...
    # content types
    CT_Unknown, CT_String, CT_Binary, CT_Real, CT_Count = range(5)
    ...
    def _update_state(self):
        ...
        mat_name = 'scg_Normal'
        state = self.getState()
        if state is Object.OS_Selected:
            mat_name = 'scg_Selected'
        elif state is Object.OS_Highlighted:
            mat_name = 'scg_Highlighted'
    ...
    def _deleteObject(self, object):
        ...
        """
        objects = [] + object.linkedObjects[Object.LS_IN] + object.linkedObjects[Object.LS_OUT]
        ...
        # if object selected, then remove it from selected objects list
        if object._getSelected():
...
#            v = v / scale
    ...
    def getContent(self):
        ...
        @return: (content type, content data)
        """
        if self.eventContentUpdate:
            self.eventContentUpdate()
        return (self.content_type, self.content_data)
...
#        self.border = SceneNode
    ...
    def addChild(self, child):
        ...
        if isinstance(child, ObjectSheet):
...
class ObjectText(Object):
    ...
    def _updateWidgets(self):
        ...
        self.__panel.setSize(sz.width + 24, sz.height + 24)
    ...
    def update_text_value(self):
        ...
        self.__text.setCaption(unicode(self.textValue))
        self._updateWidgets()
#        sz = self.__text.getTextSize()
#        self.__text.setSize(sz.width, sz.height)
    ...
    def _create_text(self):
        ...
        self.__text = self.__panel.createWidgetT("StaticText", "Idtf", mygui.IntCoord(12, 12, 0, 0), mygui.Align())
